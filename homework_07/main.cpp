...
struct Coord {
  ...
  auto operator==(const Coord& o) const -> bool { return std::fabs(x - o.x) < 1e-6f && std::fabs(y - o.y) < 1e-6f; }

  [[nodiscard]] auto length() const -> float { return std::hypot(x, y); }

  [[nodiscard]] auto normalize() const -> Coord
  {
    float len = length();
    if (len < 1e-6f) {
...
public:
  virtual auto getTargetCount() -> int = 0;
  virtual auto getTarget(int idx) -> Target = 0;
...
public:
  ...
  virtual auto solve(const Coord& dronePos,
                     float droneDir,
                     float droneSpeed,
                     const Target& target,
                     float currentTime,
...
inline auto normalizeAngle(float a) -> float
{
  while (a > PI)
    a -= 2.0f * PI;
  while (a < -PI)
...
// a = v² / (2·s)
inline auto calcAccel(float speed, float accelPath) -> float
{
  return speed * speed / (2.0f * accelPath);
}

inline auto calcDistance(const Coord& a, const Coord& b) -> float
{
  return (b - a).length();
}

// s = v² / (2·a)
inline auto accelPathFromSpeed(float v, float a) -> float
{
  if (a < 1e-6f) {
    std::cerr << "Error: a must be > 0 in accelPathFromSpeed" << std::endl;
    return 0.0f;
  }
  return v * v / (2.0f * a);
}
...
// t = sqrt(2·d / a)
inline auto accelTimeFromDist(float d, float a) -> float
{
  if (a < 1e-6f) {
    std::cerr << "Error: a must be > 0 in accelTimeFromDist" << std::endl;
    return 0.0f;
  }
...
auto writeOutputToFile(const char* filename, const SimStep* simSteps, int n) -> bool
{
  std::fstream f(filename, std::ios::out);
  if (!f) {
    LOG("Error opening output file: " << filename);
    return false;
  }
...
class FileConfigLoader : public IConfigLoader {
  ...
  auto loadDroneConfig(const char* filename, bool& outLoaded) -> DroneConfig
  {
    std::fstream f(filename, std::ios::in);
    if (!f) {
      LOG("Error opening input file: " << filename);
      outLoaded = false;
      return {};
    }
    ...
    DroneConfig cfg = {.initialPos = {data["drone"]["position"]["x"].get<float>(), data["drone"]["position"]["y"].get<float>()},
  ...
  auto loadAmmoParamsArray(const char* filename, int& outAmmoCount) -> AmmoParams*
  {
    std::fstream f(filename, std::ios::in);
    if (!f) {
      LOG("Error opening ammo parameters file: " << filename);
      outAmmoCount = 0;
      return nullptr;
    }
    ...
    int n = data.size();
    auto arr = new AmmoParams[n];
    for (int i = 0; i < n; ++i) {
      arr[i] = {.mass = data[i]["mass"].get<float>(), .drag = data[i]["drag"].get<float>(), .lift = data[i]["lift"].get<float>()};
...
public:
  ...
  auto load() -> bool override
  {
    ...
    delete[] arr;
    if (!found) {
      LOG("Error: unknown ammo type: " << cfg_.ammoName);
      return false;
    }

    if (std::fabs(cfg_.initialDir) > PI) {
      LOG("Error: initialDir out of [-π,π]");
      return false;
    }
    if (cfg_.attackSpeed <= 0.0f) {
      LOG("Error: attackSpeed must be > 0");
      return false;
    }
    if (cfg_.accelPath <= 0.0f) {
      LOG("Error: accelPath must be > 0");
      return false;
    }
    if (cfg_.arrayTimeStep <= 0.0f) {
      LOG("Error: arrayTimeStep must be > 0");
      return false;
    }
    if (cfg_.simTimeStep <= 0.0f) {
      LOG("Error: simTimeStep must be > 0");
      return false;
    }
    if (cfg_.hitRadius <= 0.0f) {
      LOG("Error: hitRadius must be > 0");
      return false;
    }
    if (cfg_.angularSpeed <= 0.0f) {
      LOG("Error: angularSpeed must be > 0");
      return false;
    }
    if (std::fabs(cfg_.turnThreshold) > PI) {
      LOG("Error: turnThreshold out of [-π,π]");
      return false;
    }
  ...
  [[nodiscard]] auto getConfig() const -> DroneConfig override { return cfg_; }
  [[nodiscard]] auto getAmmoParams() const -> AmmoParams override { return ammo_; }
};
...
class JsonTargetProvider : public ITargetProvider {
  ...
  auto loadTargetsCoordinatesArray(const char* filename, int& outCount, int& outSteps) -> Coord**
  {
    std::fstream f(filename, std::ios::in);
    if (!f) {
      LOG("Error opening targets file: " << filename);
      outCount = outSteps = 0;
      return nullptr;
    }
    ...
    auto arr = new Coord*[count];
    for (int i = 0; i < count; ++i) {
      arr[i] = new Coord[steps];
      for (int j = 0; j < steps; ++j)
        arr[i][j] = {data["targets"][i]["positions"][j]["x"].get<float>(), data["targets"][i]["positions"][j]["y"].get<float>()};
    }
    outCount = count;
    outSteps = steps;
    return arr;
...
public:
  JsonTargetProvider(const char* filename) { arr_ = loadTargetsCoordinatesArray(filename, count_, steps_); }
  ...
  auto getTargetCount() -> int override { return count_; }
  auto getTarget(int idx) -> Target override { return {arr_[idx], steps_}; }
};
...
class AnalyticalSolver : public IBallisticSolver {
  ...
  {
    float a = ammo.drag * GRAVITY * ammo.mass - 2.0f * ammo.drag * ammo.drag * ammo.lift * cfg.attackSpeed;
    if (std::fabs(a) < 1e-6f) {
      LOG("Error: coeff a ~ 0, a = " << a);
      return false;
    }

    float b = -3.0f * GRAVITY * ammo.mass * ammo.mass + 3.0f * ammo.drag * ammo.lift * ammo.mass * cfg.attackSpeed;
    float c = 6.0f * ammo.mass * ammo.mass * cfg.altitude;
    float p = -b * b / (3.0f * a * a);
    float q = 2.0f * b * b * b / (27.0f * a * a * a) + c / a;

    if (p >= 0) {
      LOG("Error: p must be negative, p = " << p);
      return false;
    }

    float arg = 3.0f * q / (2.0f * p) * std::sqrt(-3.0f / p);
    if (arg < -1.0f - 1e-6f || arg > 1.0f + 1e-6f) {
      LOG("Error: arccos arg out of [-1,1]: " << arg);
      return false;
    }
    ...
    if (t < 0) {
      LOG("Error: negative flight time, t = " << t);
      return false;
    }
  ...
  auto calcAmmoHDistance(const DroneConfig& cfg, const AmmoParams& ammo, float t) -> float
  {
    ...
    return V * t - std::pow(t, 2) * d * V / (2.0f * m) +
           std::pow(t, 3) * (6.0f * d * GRAVITY * l * m - 6.0f * d2 * (l2 - 1.0f) * V) / (36.0f * m2) +
           std::pow(t, 4) *
             (-6.0f * d2 * GRAVITY * l * (1.0f + l2 + l2 * l2) * m + 3.0f * d3 * l2 * lp * V + 6.0f * d3 * l2 * l2 * lp * V) /
             (36.0f * lp * lp * m3) +
  ...
  void getIntermediateAndDropPoint(const Coord& dronePos,
  ...
  auto interpolate(const Target& target, float t, float arrayTimeStep) -> Coord
  {
    ...
    return {pos[idx].x + (pos[next].x - pos[idx].x) * frac, pos[idx].y + (pos[next].y - pos[idx].y) * frac};
  }

  // Прогнозує де буде ціль через dt секунд від currentTime.
  // прогноз = поточна_позиція + швидкість * dt
  auto extrapolate(const Target& target, float currentTime, float dt, float arrayTimeStep) -> Coord
  {
    int n = target.timeStepsCount;
  ...
  auto calcTimeOfFlight(Coord currentPos,
                         const Coord& targetPos,
                         float currentDir,
                         ...
                         float accelPath) -> float
  {
    ...
    float accelDist = std::max(0.0f, std::min(accelPathFromSpeed(maxSpeed, a) - accelPathFromSpeed(currentSpeed, a), dist));
...
public:
             ...
             Coord& outAimPoint) -> bool override
  {
    ...
    for (int iter = 0; iter < NUM_APPROX_STEPS; ++iter) {
      ...
      outPredictedTarget = extrapolate(target, currentTime, prevTotal + ammoFlightTime_, cfg.arrayTimeStep);

      getIntermediateAndDropPoint(dronePos, outPredictedTarget, hDist_, cfg.accelPath, outIntermediatePos, outFirePos, outHasIntermediate);
      ...
      if (outHasIntermediate) {
        totalTime = calcTimeOfFlight(
          dronePos, outIntermediatePos, droneDir, cfg.turnThreshold, cfg.angularSpeed, droneSpeed, cfg.attackSpeed, cfg.accelPath);
        float dirToFire = std::atan2(outFirePos.y - outIntermediatePos.y, outFirePos.x - outIntermediatePos.x);
        totalTime += std::fabs(normalizeAngle(dirToFire - droneDir)) / cfg.angularSpeed;
        totalTime += calcTimeOfFlight(
          outIntermediatePos, outFirePos, dirToFire, cfg.turnThreshold, cfg.angularSpeed, cfg.attackSpeed, cfg.attackSpeed, cfg.accelPath);
      }
      else {
        totalTime =
          calcTimeOfFlight(dronePos, outFirePos, droneDir, cfg.turnThreshold, cfg.angularSpeed, droneSpeed, cfg.attackSpeed, cfg.accelPath);
      }

      if (std::fabs(totalTime - prevTotal) < TIME_CHANGE_THRESHOLD)
        break;
    }
...
auto createSolver(SolverType type, const DroneConfig& cfg, const AmmoParams& ammo) -> IBallisticSolver*
{
  switch (type) {
...
auto createProvider(ProviderType type, const char* filename) -> ITargetProvider*
{
  switch (type) {
...
auto createLoader(LoaderType type, const char* configFile, const char* ammoFile) -> IConfigLoader*
{
  switch (type) {
    case LoaderType::FILE:
      return new FileConfigLoader(configFile, ammoFile);
  }
  return nullptr;
}
...
public:
  ...
  [[nodiscard]] auto hasNext() const -> bool { return initialized_ && !done_ && recordedSteps_ < MAX_STEPS; }
  ...
  auto step() -> SimStep
  {
    ...
    for (int i = 0; i < targets_->getTargetCount(); ++i) {
      ...
      bool alreadyApproaching = (i == currentTargetIdx_) && (droneState_ == MOVING || droneState_ == ACCELERATING);

      if (!solver_->solve(currentPos_,
                          currentDir_,
                          currentSpeed_,
                          targets_->getTarget(i),
                          currentTime_,
                          alreadyApproaching,
                          cfg_,
                          ammo_,
    ...
    float desiredDir = std::atan2(bestFirePos.y - currentPos_.y, bestFirePos.x - currentPos_.x);
    ...
    DEBUG("Step: " << recordedSteps_ << ", Time: " << currentTime_ << ", Pos: (" << currentPos_.x << ", " << currentPos_.y << ")"
                   << ", Dir: " << currentDir_ << ", State: " << static_cast<int>(droneState_) << ", Target: " << currentTargetIdx_);

    // STEP 6: перевірка умови скидання
    if (droneState_ == MOVING && calcDistance(currentPos_, bestFirePos) <= cfg_.hitRadius * 0.2f)
      done_ = true;
  ...
  auto writeOutput(const char* filename) const -> bool { return writeOutputToFile(filename, steps_, recordedSteps_); }

  [[nodiscard]] auto getRecordedSteps() const -> int { return recordedSteps_; }
...
auto main() -> int
{
  ...
  IBallisticSolver* solver = createSolver(SolverType::ANALYTICAL, loader->getConfig(), loader->getAmmoParams());
